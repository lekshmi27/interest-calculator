        ...
        <div className="d-flex justify-content-center align-items-center bg-warning p-3 rounded shadow text-light flex-column">
          ...
          <form className='mt-5' >
            ...
            <TextField className='w-100' id="outlined-basic-principle" label="₹ Principle Amount" variant="outlined"
             value={principle || ""} onChange={e=> handleValidation(e.target)} name='principle' />
            </div>
            ...
            <TextField className='w-100' id="outlined-basic-rate" label="Rate of Interest (p.a) %" variant="outlined"
             value={rate || ""} onChange={e=> handleValidation(e.target)} name='rate' />
            </div>
            ...
            <TextField className='w-100' id="outlined-basic-time" label="Time Period (Yr)" variant="outlined"
             value={year|| ""} onChange={e=> handleValidation(e.target)} name='year' />
            </div>
            {!yearAmountValid && <div className="text-danger mb-3">*Invalid User Input</div>}

            <Stack direction="row" spacing={2}>
            <Button onClick={handleCalculate} disabled={!principleAmountValid || !rateAmountValid || !yearAmountValid} style={{width:`50%`,height:`70px`}} className='bg-primary' variant="contained">Calculate</Button>
